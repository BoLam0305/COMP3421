...
if ($_POST['email'] != "" && $_POST['password'] != "") {
    ...
    $_STATEMENT = $conn->prepare("SELECT * FROM users WHERE email = ?;");
    $_STATEMENT->bind_param('s', $_POST['email']);
    $_STATEMENT->execute();
    ...
    if ($result->num_rows == 1) {
        ...
        extract($result);
        mysqli_close($conn);
        if(password_verify($unhash, $password)){
            if ($status != "Disable") {
                if ($userTypeID == 0) {
                    $_SESSION['ID'] = $userID;
            ...
            header("location:/HTML/User_Page/login.php?Empty= Invalid email and password");
        }
    } else {
        header("location:/HTML/User_Page/login.php?Empty= Invalid email and password");
    }
